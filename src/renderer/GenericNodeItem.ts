...
import { Point, Point$, PointObject, Coordinate, CircleItem, PointTextItem, PolylineItem, RegularPolygonItem, GroupItem, PaperItemObject } from 'paper-vueify'
import { ITEM_SIZES, ACTION_STROKES, ACTION_BRUSHES } from './Defs'
import { GenericItemMixin } from './GenericItem'
import { TreeNode, UniqueAction } from '../model'

@Component
export class GenericNodeItemMixin extends Mixins(GenericItemMixin) {
  ...
  get width() {
    if (this.isLeaf) {
      return ITEM_SIZES.DIAMETER / (this.item.value != null ? 1 : 2)
    } else {
  ...
  get link() {
    if (this.parentPosition) {
      let parent = (this.item as TreeNode<any>).parent
      let terminal = Point$.Subtract(this.parentPosition, this.position)
      let xFrom = this.diameter / 2
      let xTo = Point$.Length(terminal) - Math.max(this.GetDiameter(parent.value, parent.attribute), ITEM_SIZES.DIAMETER) / 2
      let isParentRotating = parent.action === UniqueAction.Rotate
      ...
      if (!isRotating && isParentRotating && parent.children.some(c => !!c && c.action === UniqueAction.Rotate)) {
  ...
  beforeDestroy() {
    let tag = (this.item as TreeNode<any>).tag
    if (tag > -1) {
      this.$emit('tag', tag)
    }
  }
